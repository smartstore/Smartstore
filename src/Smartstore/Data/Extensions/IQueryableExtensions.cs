...
public static class IQueryableExtensions
{
    ...
    extension<T>(IQueryable<T> query) where T : BaseEntity
    {
        /// <summary>
        /// Applies "AsTracking()" or "AsNoTracking()" according to <paramref name="tracked"/> parameter.
        /// </summary>
        public IQueryable<T> ApplyTracking(bool tracked)
        {
            Guard.NotNull(query);
        ...
        public IQueryable<T> ApplyIdFilter(IEnumerable<int> ids)
        {
            Guard.NotNull(query);
            Guard.NotNull(ids);
        ...
        /// </remarks>
        public IQueryable<T> SelectSummary()
        {
            Guard.NotNull(query);
        ...
        /// </summary>
        public FastPager<T> ToFastPager(int pageSize = 1000)
        {
            Guard.NotNull(query);
        ...
        /// </summary>
        public IList<T> GetMany(IEnumerable<int> ids, bool tracked = false)
        {
            Guard.NotNull(query);
            Guard.NotNull(ids);
        ...
        /// Sort is applied in-memory.
        /// </summary>
        public async Task<List<T>> GetManyAsync(IEnumerable<int> ids, bool tracked = false)
        {
            Guard.NotNull(query);
            Guard.NotNull(ids);
        ...
        public int ExecuteDelete(int bulkSize)
        {
            Guard.NotNull(query);
        ...
        /// </summary>
        /// <param name="bulkSize">Number of rows to be deleted in a single delete operation.</param>
        public async Task<int> ExecuteDeleteAsync(int bulkSize, CancellationToken cancellationToken = default)
        {
            Guard.NotNull(query);
            Guard.IsPositive(bulkSize);
    ...
    extension (IQueryable query)
    {
        ...
        public HookingDbContext GetDbContext()
        {
            Guard.NotNull(query);
