        ...
        public override void Up()
        {
            ...
            if (!Schema.Table(ShippingByTotal).Exists())
            {
                Create.Table(ShippingByTotal)
                    ...
                    .WithColumn(nameof(ShippingRateByTotal.From)).AsDecimal(2, 18).NotNullable() // WTF ???!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    .WithColumn(nameof(ShippingRateByTotal.To)).AsDecimal(2, 18).Nullable()
                    .WithColumn(nameof(ShippingRateByTotal.UsePercentage)).AsBoolean()
                    .WithColumn(nameof(ShippingRateByTotal.ShippingChargePercentage)).AsDecimal(2, 18).NotNullable()
                    .WithColumn(nameof(ShippingRateByTotal.ShippingChargeAmount)).AsDecimal(2, 18).NotNullable()
                    .WithColumn(nameof(ShippingRateByTotal.BaseCharge)).AsDecimal(2, 18).NotNullable()
                    .WithColumn(nameof(ShippingRateByTotal.MaxCharge)).AsDecimal(2, 18).Nullable();
            }
        }
