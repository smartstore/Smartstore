    ...
    public class PollAdminController : AdminController
    {
        ...
        public async Task<IActionResult> PollList(GridCommand command)
        {
            ...
            var pollModels = await polls
                .SelectAsync(async x =>
                {
                    ...
                    {
                        model.EndDate = _dateTimeHelper.ConvertToUserTime(x.EndDateUtc.Value, DateTimeKind.Utc).ToShortDateString();
                    }
        ...
        public async Task<IActionResult> Create(PollModel model, bool continueEditing)
        {
            if (ModelState.IsValid)
            {
                ...
                poll.StartDateUtc = model.StartDateUtc.HasValue
                    ? _dateTimeHelper.ConvertToUtcTime(model.StartDateUtc.Value)
                    : null;

                poll.EndDateUtc = model.EndDateUtc.HasValue
                    ? _dateTimeHelper.ConvertToUtcTime(model.EndDateUtc.Value)
                    : null;
                ...
                return continueEditing 
                    ? RedirectToAction(nameof(Edit), new { id = poll.Id }) 
        ...
        public async Task<IActionResult> Edit(PollModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                await _storeMappingService.ApplyStoreMappingsAsync(poll, model.SelectedStoreIds);
                await _db.SaveChangesAsync();
        ...
        [HttpPost]
        [Permission(PollPermissions.Delete)]
        public async Task<IActionResult> PollDelete(GridSelection selection)
        {
            var success = false;
            var numDeleted = 0;
        ...
        public async Task<IActionResult> PollAnswerList(int? pollId, GridCommand command)
        {
            ...
            var answersModel = answers.Select(x => new PollAnswerModel
            {
                Id = x.Id,
                PollId = x.PollId,
                Name = x.Name,
        ...
        public async Task<IActionResult> PollVotesList(int pollId, GridCommand command)
        {
            ...
            var votingModel = votings.Select(x => new PollVotingRecordModel
            {
                Id = x.Id,
                ...
                Username = x.Customer.GetDisplayName(T),
                CustomerEditUrl = Url.Action("Edit", "Customer", new { id = x.CustomerId, area = "Admin" })
            });
