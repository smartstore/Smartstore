...
namespace Smartstore.Web.Api
{
    ...
    public class ApiUserStore : IApiUserStore
    {
        ...
        private DateTime _lastSavedDate = DateTime.UtcNow;
        ...
        internal async Task<int> SaveApiUsersInternal(Dictionary<string, WebApiUser> users, bool async)
        {
            ...
            if (users != null)
            {
                var usersToStore = users.Values.Where(x => x.LastRequest.HasValue && x.LastRequest > _lastSavedDate && x.IsValid).ToList();
                if (usersToStore.Count > 0)
                {
                    ...
                    _lastSavedDate = DateTime.UtcNow;
                }
            }
