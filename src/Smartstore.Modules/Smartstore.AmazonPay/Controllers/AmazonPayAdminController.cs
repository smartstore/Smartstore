    ...
    public async Task<IActionResult> Configure(AmazonPaySettings settings)
    {
        ...
        try
        {
            foreach (var entity in allStores)
            {
                ...
                if (baseUrl != null)
                {
                    model.MerchantLoginDomains.Add(baseUrl);
                    if (entity.Id == store.Id)
                    {
                        model.CurrentMerchantLoginDomains.Add(baseUrl);
                    }
                }
                ...
                if (entity.Id == store.Id)
                {
                    ...
                    model.KeyShareUrl = CreateUrl(nameof(AmazonPayController.ShareKey), baseUri);
                    model.IpnUrl = CreateUrl(nameof(AmazonPayController.IPNHandler), baseUri);
                    ...
                    {
                        model.MerchantPrivacyNoticeUrl = WebHelper.GetAbsoluteUrl(topicUrl, Request, true, baseUri.Scheme);
                    }
                }
            }
        }
        ...
        string CreateUrl(string action, Uri uri)
        {
            return Url.Action(action, "AmazonPay", new { area = string.Empty }, uri.Scheme, uri.Authority).TrimEnd('/');
        }
    }
