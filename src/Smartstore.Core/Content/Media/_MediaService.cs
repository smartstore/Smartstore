...
//            var q = await _searcher.PrepareQueryAsync(query, MediaLoadFlags.None);
...
//            var untrackableFolderIds = (await _folderService.GetRootNodeAsync())
...
//            {
//                await EnsureMetadataResolvedAsync(entity, true);
//                return ConvertMediaFile(entity, (await _folderService.FindNodeAsync(entity))?.Value);
//            }
...
//            {
//                await EnsureMetadataResolvedAsync(entity, true);
//                var node = await _folderService.FindNodeAsync(entity);
...
//        public async Task<List<MediaFileInfo>> GetFilesByIds(int[] ids, MediaLoadFlags flags = MediaLoadFlags.AsNoTracking)
//        {
...
//            var query = (await _searcher.PrepareQueryAsync(q, MediaLoadFlags.AsNoTracking)).Select(x => x.Name);
...
//        protected async Task<MediaStorageItem> ProcessFile(
//            ref MediaFile file,
...
//                : _db.Table.FirstOrDefault(x => x.Name == destFileName && x.FolderId == destFolderId);
...
//            using (var scope = new DbContextScope(_db.Context, autoCommit: false))
//            {
//                // Tags.
//                _db.Context.LoadCollection(file, (MediaFile x) => x.Tags);
...
//                scope.Commit();
//                _db.Context.DetachEntities<MediaTag>();
//            }
//        }
...
//            else
//            {
//                var dupe = _db.Table.FirstOrDefault(x => x.Name == destFileName && x.FolderId == destFolderId);
//                if (dupe != null)
//                {
//                    if (!folderChanged)
//                    {
