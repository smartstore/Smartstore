...
namespace Smartstore.Core.Catalog.Discounts
{
        ...
        Lazy<IShoppingCartService> cartService) : AsyncDbSaveHook<Discount>, IDiscountService
    {
        ...
        class DiscountKey(Discount discount, Customer customer, string customerCouponCode, Store store, DiscountValidationFlags flags) 
            : Tuple<Discount, Customer, string, Store, DiscountValidationFlags>(discount, customer, customerCouponCode, store, flags)
        {
        }
    }
}
