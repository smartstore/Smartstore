...
namespace Smartstore.Core.Catalog.Products
{
    public partial class ProductCloner : IProductCloner
    {
        ...
        {
            await _urlService.ValidateAndApplySlugAsync(clone, string.Empty, clone.GetDisplayName(), true);

            foreach (var lang in languages)
            {
                string name = product.GetLocalized(x => x.Name, lang, false, false);

                await _urlService.ValidateAndApplySlugAsync(clone, string.Empty, name, false, lang.Id, true);
            }
        }
