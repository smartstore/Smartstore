...
namespace Smartstore.Core.Rules
{
    internal class RuleHook : AsyncDbSaveHook<RuleEntity>
    {
        ...
        private readonly HashSet<int> _ruleSetIdsOfDeletedRules = new();
        ...
        public override async Task OnAfterSaveCompletedAsync(IEnumerable<IHookedEntity> entries, CancellationToken cancelToken)
        {
            if (_ruleSetIdsOfDeletedRules.Count > 0)
            {
                ...
                await _db.RuleSets
                    .Where(x => _ruleSetIdsOfDeletedRules.Contains(x.Id) && !x.Rules.Any() && x.Scope == RuleScope.ProductAttribute)
