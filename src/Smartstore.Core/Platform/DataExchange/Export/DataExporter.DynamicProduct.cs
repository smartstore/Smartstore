            ...
            DataExporterContext ctx)
        {
            ...
            if (ctx.Supports(ExportFeatures.CanIncludeMainPicture))
            {
                ...
                if (mediaFiles?.Any() ?? false)
                {
                    ...
                    dynObject._MainPictureUrl = _mediaService.GetUrl(file, imageQuery, ctx.Store.GetHost(true));
                ...
                else if (!_catalogSettings.HideProductDefaultPictures)
                {
                    ...
                    dynObject._MainPictureUrl = _mediaService.GetUrl(null, imageQuery, ctx.Store.GetHost(true));
