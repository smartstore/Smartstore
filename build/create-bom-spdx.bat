...
REM Format: SPDX JSON
REM Output: sbom-spdx.json in the artifacts directory
REM Requirements:
REM   - Windows
...
REM Build artifacts pattern: Community.{Version}.{Platform}
REM Example:
REM   COMMUNITY_VERSION=5.1.0
REM   PLATFORM=win-x64
set "COMMUNITY_VERSION=6.2.0"
set "PLATFORM=win-x64"

REM Artifacts directory relative to build directory
set "ARTIFACTS_DIR_REL=artifacts\Community.%COMMUNITY_VERSION%.%PLATFORM%"

REM Compose absolute scan path
set "SCAN_DIR=%SCRIPT_DIR%%ARTIFACTS_DIR_REL%"

REM SBOM output file name (relative to SCAN_DIR)
set "SBOM_FILE=sbom-spdx.json"
...
echo.
echo [SBOM] Creating SPDX SBOM for Smartstore build artifacts...
...
if not exist "%SCAN_DIR%" (
    ...
    echo Please adjust COMMUNITY_VERSION and PLATFORM in this script.
    goto :Fail
)
...
REM ==== Generate SPDX SBOM ====================================
echo.
echo [INFO] Generating SPDX SBOM with Syft from build artifacts...
pushd "%SCAN_DIR%" || (
    echo [ERROR] Could not change directory to "%SCAN_DIR%".
    goto :Fail
)

REM 1) Generate raw SPDX SBOM (compact JSON)
"%SYFT_EXE%" dir:. -o spdx-json=%SBOM_FILE%
...
echo [INFO] Prettifying SPDX JSON SBOM...
powershell -Command ^
  "$p='%SBOM_FILE%';"^
...
if errorlevel 1 (
    popd
    echo [ERROR] Failed to prettify SPDX JSON SBOM.
    goto :Fail
)
...
echo.
echo [OK] SPDX SBOM successfully created and prettified:
